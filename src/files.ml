...
let convV0 = Remote.makeConvV0FunRet
  (fun (b, desc) -> (b, Props.basic_to_compat251 desc))
  (fun (b, desc) -> (b, Props.basic_of_compat251 desc))
...
let convV0 = Remote.makeConvV0FunArg
  (fun (fspath, (workingDir, path, initialDesc, newDesc)) ->
       (fspath, (workingDir, path,
         Props.basic_to_compat251 initialDesc, Props.to_compat251 newDesc)))
  (fun (fspath, (workingDir, path, initialDesc, newDesc)) ->
       (fspath, (workingDir, path,
         Props.basic_of_compat251 initialDesc, Props.of_compat251 newDesc)))
      ...
      Lwt_unix.run
        (debug (fun () -> Util.msg "Committing results of merge\n");
         ...
         if Fs.file_exists arch_fspath then begin
           ...
           let fp = Os.fingerprint arch_fspath Path.empty infoarch.Fileinfo.typ in
