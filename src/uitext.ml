...
let dumbtty =
  Prefs.createBool "dumbtty"
    ...
    (s_ "do not change terminal settings in text UI")
    (s_ "When set to \\verb|true|, this flag makes the text mode user \
...
let silent =
  Prefs.createBool "silent" false
    ...
    (s_ "print nothing except error messages")
    (s_ "When this preference is set to {\\tt true}, the textual user \
...
let colorMode =
  Prefs.createBoolWithDefault "color"
    ...
    (s_ "use color output for text UI (true/false/default)")
    (s_ "When set to {\\tt true}, this flag enables color output in \
...
let display message =
  ...
  else Printf.ifprintf stdout message (* no output *)
    ...
    Safelist.iter
      (fun (names,doc,action) ->
         if (Safelist.nth names 0) = "" then
           display "%s" (formatname (Safelist.nth names 1)))
      actions;
    display "] " in
  let tryagainOrLoop() =
    tryagain ();
    ...
    if a="?" || a = "\027OP" then
      (newLine ();
       ...
       Safelist.iter (fun (names,doc,action) ->
         let n = Util.concatmap (s_ " or ") formatname names in
      ...
      | None ->
          newLine ();
          if a="" then
            display (f_ "No default command [type '?' or F1 for help]\n")
          else
            display (f_ "Unrecognized command '%s': try again  \
              [type '?' or F1 for help]\n") (String.escaped a);
          tryagainOrLoop()
  in
  let handleExn s =
    ...
    alwaysDisplay "\n";
    raise (Util.Fatal
      (Printf.sprintf (f_ "Failure reading from the standard input (%s)\n") s))
  in
  let userInput () =
    try
      Some (getInput ())
...
let alwaysDisplayErrors prefix l =
  List.iter
    (fun err -> alwaysDisplay "%s%s\n" prefix err) l

let alwaysDisplayDetails ri =
  alwaysDisplay "%s\n" (Uicommon.details2string ri "  ");
  match ri.replicas with
    Problem _ ->
      ()
  | Different diff ->
      alwaysDisplayErrors (s_ "[root 1]: ") diff.errors1;
      alwaysDisplayErrors (s_ "[root 2]: ") diff.errors2
...
let displayri ri =
  ...
  let (defaultAction, forcedAction) =
    match action with
      Uicommon.AError      -> ((color `AError) ^ (s_ "error") ^ (color `Reset), (color `AError) ^ (s_ "error") ^ (color `Reset))
  ...
  | Different {direction = d} when isConflict d ->
      alwaysDisplay
  | _ ->
      display) "%s %s %s   %s  " r1 action r2 path
...
let interact prilist rilist =
  ...
  let rec loop prev =
    let rec previous prev ril =
      ...
      | [] -> display "\n%s\n" (Uicommon.roots2string ()); loop prev ril in
    ...
    | ri::rest as ril ->
        ...
        let ignore_pref pat rest what =
          ...
          display (f_ "  Permanently ignoring %s\n") what;
          begin match !Prefs.profileName with None -> assert false |
            Some(n) ->
              display (f_ "  To un-ignore, edit %s and restart Unison\n")
                (Prefs.profilePathname n) end;
          let nukeIgnoredRis =
        ...
        let setripred cmd =
          ripred := match cmd, !ripred with
              `Unset, [] -> display (f_ "Matching condition already disabled\n"); []
            | `Unset, _ | `Pop, [_] -> display (f_ "  Disabling matching condition\n"); []
            | `Pop, p::pp::t -> pp::t
            | `Push rp, [] -> display (f_ "  Enabling matching condition\n"); [rp]
            | `Push rp, p -> rp::p
            | _, [] -> display (f_ "Matching condition not enabled\n"); []
            | `Op1 op, p::t -> (fun ri -> op (p ri))::t
            | `Op2 op, [p] -> display (f_ "Missing previous matching condition\n"); [p]
        ...
        | Different {rc1 = _; rc2 = _; direction = dir} ->
            if Prefs.read Uicommon.auto && not (isConflict dir) then begin
              ...
              let (descr, descl) =
                if host1 = host2 then
                  s_ "left to right", s_ "right to left"
                else
                  Printf.sprintf (f_ "from %s to %s") host1 host2,
                  Printf.sprintf (f_ "from %s to %s") host2 host1
              in
              ...
              selectAction
                ...
                [((if (isConflict dir) && not (Prefs.read Globals.batch)
                  ...
                  (s_ "follow Unison's recommendation (if any)"),
                  (fun () -> newLine();
                     if (isConflict dir) && not (Prefs.read Globals.batch)
                     then begin
                       display (f_ "No default action [type '?' for help]\n");
                       repeat()
                     end else
                       next()));
                 (["n";"j"; keyDn; keyTab],
                  (s_ "go to the next item"),
                  (fun () -> newLine();
                     next()));
                 (["p";"b";"k"; keyUp; keyRvTab],
                  (s_ "go back to previous item"),
                  (fun () -> newLine();
                     previous prev ril));
                 (["\x7f";"\b"; keyDelete],
                  (s_ "revert then go back to previous item"),
                  (fun () ->
                     Recon.revertToDefaultDirection ri; redisplayri();
                     previous prev ril));
                 (["0"; keyHome],
                  (s_ "go to the start of the list"),
                  (fun () -> newLine();
                     loop [] (Safelist.rev_append prev ril)));
                 (["9"; keyEnd],
                  (s_ "go to the end of the list"),
                  (fun () -> newLine();
                     match Safelist.rev_append ril prev with
                       [] -> loop [] []
                     | lri::prev -> loop prev [lri]));
                 (["5"; keyPgDn],
                  (s_ "go forward to the middle of the following items"),
                  (fun () -> newLine();
                     let l = (Safelist.length ril)/2 in
                     display (fn_ "  Moving %d item forward\n" "  Moving %d items forward\n" l) l;
                     forward l prev ril));
                 (["6"; keyPgUp],
                  (s_ "go backward to the middle of the preceding items"),
                  (fun () -> newLine();
                     let l = -((Safelist.length prev)+1)/2 in
                     display (fn_ "  Moving %d item backward\n" "  Moving %d items backward\n" l) l;
                     forward l prev ril));
                 (["R"],
                  (s_ "reverse the list of paths"),
                  (fun () -> newLine();
                     loop rest (ri::prev)));
                 (["d"],
                  (s_ "show differences (curr or match)"),
                  (fun () ->
                     actOnMatching ~change:false showdiffs));
                 (["x"],
                  (s_ "show details (curr or match)"),
                  (fun () ->
                      actOnMatching ~change:false
                        (fun ri -> displayDetails ri; true)));
                 (["L"],
                  (s_ "list all (or matching) following changes tersely"),
                  (fun () -> newLine();
                     Safelist.iter
                     ...
                     repeat()));
                 (["l"],
                  (s_ "list all (or matching) following changes with details"),
                  (fun () -> newLine();
                     Safelist.iter
                       (fun ri -> display "  "; displayri ri; display "\n";
                     ...
                     repeat()));
                 (["A";"*"],
                  (s_ "match all the following"),
                  (fun () -> newLine();
                     setripred (`Push (fun _ -> true));
                     repeat()));
                 (["1"],
                  (* TRANSLATORS: %s will be "left to right" or "from <host1>
                     to <host2>". *)
                  (Printf.sprintf (f_ "match all the following that propagate %s") descr),
                  (fun () -> newLine();
                     ...
                     repeat()));
                 (["2"],
                  (* TRANSLATORS: %s will be "right to left" or "from <host1>
                     to <host2>". *)
                  (Printf.sprintf (f_ "match all the following that propagate %s") descl),
                  (fun () -> newLine();
                     setripred (`Push (function
                     ...
                     repeat()));
                 (["C"],
                  (s_ "match all the following conflicts"),
                  (fun () -> newLine();
                     setripred (`Push (function
                         {replicas = Different ({direction = Conflict _})} -> true
                       | _ -> false));
                     repeat()));
                 (["P";"="],
                  (s_ "match all the following with only props changes"),
                  (fun () -> newLine();
                     setripred (`Push ispropschanged);
                     repeat()));
                 (["M"],
                  (s_ "match all the following merges"),
                  (fun () -> newLine();
                     setripred (`Push (function
                     ...
                     repeat()));
                 (["X";"!"],
                  (s_ "invert the matching condition"),
                  (fun () -> newLine();
                     setripred (`Op1 not);
                     repeat()));
                 (["&"],
                  (s_ "and the last two matching conditions"),
                  (fun () -> newLine();
                     setripred (`Op2 (&&));
                     repeat()));
                 (["|"],
                  (s_ "or the last two matching conditions"),
                  (fun () -> newLine();
                     setripred (`Op2 (||));
                     repeat()));
                 (["D";"_"],
                  (s_ "delete/pop the active matching condition"),
                  (fun () -> newLine();
                     setripred `Pop;
                     repeat()));
                 (["U";"$"],
                  (s_ "unmatch (select current)"),
                  (fun () -> newLine();
                     setripred `Unset;
                     repeat()));
                 (["r";"u"],
                  (s_ "revert to Unison's default recommendation (curr or match)"),
                  (fun () ->
                     actOnMatching
                       (fun ri->Recon.revertToDefaultDirection ri; true)));
                 (["m"],
                  (s_ "merge the versions (curr or match)"),
                  (fun () ->
                     actOnMatching (setdir Merge)));
                 ([">";"."; keyRight],
                  (* TRANSLATORS: %s will be "left to right" or "from <host1>
                     to <host2>". *)
                  (Printf.sprintf (f_ "propagate from %s (curr or match)") descr),
                  (fun () ->
                     actOnMatching (setdir Replica1ToReplica2)));
                 (["<";","; keyLeft],
                  (* TRANSLATORS: %s will be "right to left" or "from <host1>
                     to <host2>". *)
                  (Printf.sprintf (f_ "propagate from %s (curr or match)") descl),
                  (fun () ->
                     actOnMatching (setdir Replica2ToReplica1)));
                 (["]";"\""],
                  (s_ "resolve conflicts in favor of the newer (curr or match)"),
                  (fun () ->
                     actOnMatching (setDirectionIfConflict `Newer)));
                 (["[";"'"],
                  (s_ "resolve conflicts in favor of the older (curr or match)"),
                  (fun () ->
                     actOnMatching (setDirectionIfConflict `Older)));
                 (["c"],
                  (s_ "resolve conflicts in favor of changed (curr or match)"),
                  (fun () ->
                     actOnMatching
                       ~fail:(Some (fun()->display (f_ "Cannot set direction\n")))
                       setdirchanged));
                 (["i"],
                  (s_ "invert direction of propagation (curr or match)"),
                  (fun () ->
                     actOnMatching
                       ~fail:(Some (fun()->display (f_ "Cannot invert direction\n")))
                       invertdir));
                 (["/";":"],
                  (s_ "skip (curr or match)"),
                  (fun () ->
                     actOnMatching setskip));
                 (["%"],
                  (s_ "skip all the following"),
                  (fun () -> newLine();
                     Safelist.iter (fun ri -> ignore (setskip ri); ()) rest;
                     repeat()));
                 (["-"],
                  (s_ "skip and discard for this session (curr or match)"),
                  (fun () ->
                     actOnMatching ~fail:None (fun _->false)));
                 (["+"],
                  (s_ "skip and discard all the following"),
                  (fun () -> newLine();
                     loop prev [ri]));
                 (["I"],
                  (s_ "ignore this path permanently"),
                  (fun () -> newLine();
                     ignore_pref (Uicommon.ignorePath ri.path1) rest
                       (s_ "this path")));
                 (["E"],
                  (s_ "permanently ignore files with this extension"),
                  (fun () -> newLine();
                     ignore_pref (Uicommon.ignoreExt ri.path1) rest
                       (s_ "files with this extension")));
                 (["N"],
                  (s_ "permanently ignore paths ending with this name"),
                  (fun () -> newLine();
                     ignore_pref (Uicommon.ignoreName ri.path1) rest
                       (s_ "files with this name")));
                 (["s"],
                  (s_ "stop reconciling and go to the proceed menu"),
                  (fun () -> newLine();
                     (ConfirmBeforeProceeding, Safelist.rev_append prev ril)));
                 (["g"],
                  (s_ "proceed immediately to propagating changes"),
                  (fun () -> newLine();
                     (ProceedImmediately, Safelist.rev_append prev ril)));
                 (["q"; keyEsc],
                  (s_ "exit Unison without propagating any changes"),
                  (fun () -> newLine();
                     raise Sys.Break))
                ]
                (fun () -> displayri ri)
...
let verifyMerge title text =
  ...
  if Prefs.read Globals.batch then
    ...
    if Prefs.read Uicommon.confirmmerge then begin
      display (f_ "Commit results of merge? ");
      selectAction
        ...
        [(["y";"g"],
          ...
          (fun() -> newLine();
             true));
         (["n"],
          s_ "No: leave this file unchanged",
          (fun () -> newLine();
             false));
        ]
        (fun () -> display (f_ "Commit results of merge? "))
    end else
      true
  end
...
let setWarnPrinterForInitialization()=
  Util.warnPrinter :=
    Some (fun s -> alwaysDisplay (f_ "Warning: "); alwaysDisplay "%s\n\n" s)

let setWarnPrinter() =
  Util.warnPrinter :=
    Some(fun s ->
           Util.set_infos "";
           alwaysDisplay (f_ "Warning: ");
           alwaysDisplay "%s\n" s;
           if not (Prefs.read Globals.batch) then begin
             display (f_ "Press return to continue.");
             selectAction None
               [(["";"";" ";"y"],
                 (s_ "Continue"),
                 (fun () -> newLine()));
               ...
               (fun () -> display (f_ "Press return to continue."))
           end)
...
let rec interactAndPropagateChanges prevItemList reconItemList
  ...
  if not (Prefs.read Trace.terse) && (updatesToDo > 0 || skipped > 0) then begin
    let root1, root2 =
      ...
      | (Local, _), (Remote host, _) -> s_ "local", host
      | (Remote host, _), (Local, _) -> host, s_ "local"
      | (Remote host1, _), (Remote host2, _) -> host1, host2
    in
    Trace.log_color (Printf.sprintf
      (* TRANSLATORS: The %s in groups %s%d%s will be replaced by control
         codes for outputting color. The arguments in sequence "%s, %s" are
         "... items will be synced" and "... skipped" (both translated
         separately). *)
      (f_ "\n%s, %s\n\
       %s to be synced from %s to %s\n\
       %s to be synced from %s to %s\n")
      (Printf.sprintf
        (fn_ "%s%d%s item will be synced" "%s%d%s items will be synced" updatesToDo)
        (color `Focus) updatesToDo (color `Reset))
      (Printf.sprintf
        (fn_ "%s%d%s skipped" "%s%d%s skipped" skipped)
  ...
  let doTransp () =
    ...
    with e ->
      let origbt = Printexc.get_raw_backtrace () in
      let summary =
        (* TRANSLATORS: The first and third %s will be replaced by control
           codes for outputting color. The second %s will be either
           "interrupted" or "failed". The fourth %s will be either time in form
           " at %02d:%02d:%02d" or empty. The fifth %s will be either " by user
           request" or " due to a fatal error". *)
        Printf.sprintf (f_ "\nSynchronization %s%s%s%s%s\n\n")
          (color `Failure)
          (match e with Sys.Break -> s_ "interrupted" | _ -> s_ "failed")
          (color `Reset)
          (try let tm = Util.localtime (Util.time ()) in
           Printf.sprintf (f_ (* TRANSLATORS: hours:minutes:seconds *) " at %02d:%02d:%02d")
             tm.Unix.tm_hour tm.Unix.tm_min tm.Unix.tm_sec with _ -> "")
          (match e with Sys.Break -> s_ " by user request" | _ -> s_ " due to a fatal error")
      in
      Util.set_infos "";
  ...
  let doit() =
    ...
    if not (Prefs.read Trace.terse) then Trace.status (s_ "Propagating updates");
    ...
    if not (Prefs.read Trace.terse) then Trace.status (s_ "Saving synchronizer state");
    Update.commitUpdates ();
    let trans = updatesToDo - notstarted - failures in
    let summary =
      (* TRANSLATORS: The first %s will be either "complete" or "incomplete".
         The second %s will be time in form "%02d:%02d:%02d". The third %s is
         "%d items". The fourth %s will be either empty or "%d partially
         transferred, ". The fifth, sixth, seventh %s will be "%d skipped" "%d
         failed" "%d not started", or possibly empty. *)
      Printf.sprintf
       (f_ "Synchronization %s at %s  (%s transferred, %s%s, %s%s)")
       (if failures = 0 && notstarted = 0 then (color `Success) ^ (s_ "complete") ^ (color `Reset)
        else (color `Failure) ^ (s_ "incomplete") ^ (color `Reset))
       (let tm = Util.localtime (Util.time()) in
        Printf.sprintf (f_ (* TRANSLATORS: hours:minutes:seconds *) "%02d:%02d:%02d")
          tm.Unix.tm_hour tm.Unix.tm_min tm.Unix.tm_sec)
       (Printf.sprintf (fn_ "%d item" "%d items" trans) trans)
       (if partials <> 0 then
          Format.sprintf (fn_ "%d partially transferred, " "%d partially transferred, " partials) partials
        else
          "")
       (if skipped = 0 then s_ "0 skipped" else (color `Information) ^ (Printf.sprintf (fn_ "%d skipped" "%d skipped" skipped) skipped) ^ (color `Reset))
       (if failures = 0 then s_ "0 failed" else (color `Failure) ^ (Printf.sprintf (fn_ "%d failed" "%d failed" failures) failures) ^ (color `Reset))
       (if notstarted = 0 then "" else ", " ^ (color `Information) ^ (Printf.sprintf (fn_ "%d not started" "%d not started" notstarted) notstarted) ^ (color `Reset)) in
    Trace.log_color (summary ^ "\n");
    if skipped>0 then
      Safelist.iter
        (fun ri ->
         ...
         | Different {rc1 = _; rc2 = _; direction = Conflict r; default_direction = _} ->
            alwaysDisplayAndLog (Printf.sprintf (f_ "  skipped: %s (%s)")
    ...
    if partials>0 then
      Safelist.iter
        (fun p ->
           alwaysDisplayAndLog (Printf.sprintf (f_ "  partially transferred: %s") (Path.toString p)))
        partialPaths;
    if failures>0 then
      Safelist.iter
        (fun p -> alwaysDisplayAndLog (Printf.sprintf (f_ "  failed: %s") (Path.toString p)))
        failedPaths;
  ...
  if updatesToDo = 0 then begin
    ...
    if !Update.foundArchives && Prefs.read Uicommon.repeat = `NoRepeat then
      Update.commitUpdates ();
    display (f_ "No updates to propagate\n");
    if skipped > 0 then begin
      let summary =
        Printf.sprintf
          (fn_
            (* TRANSLATORS: All %s, except "at %s", will be replaced by control
               codes for outputting color. *)
    ...
    let rec askagain newReconItemList =
      displayWhenInteractive ("\n" ^^ (f_ "Proceed with propagating updates? "));
      selectAction
        ...
        [(["y";"g"],
          s_ "Yes: proceed with updates as selected above",
          doit);
         (["n"],
          s_ "No: go through reconciliation process again",
          (fun () -> newLine();
             Prefs.set Uicommon.auto false;
             interactAndPropagateChanges [] newReconItemList));
         (["p";"b"],
          s_ "go back to the last item of the reconciliation",
          (fun () -> newLine();
             Prefs.set Uicommon.auto false;
        ...
        (fun () -> display (f_ "Proceed with propagating updates? "))
...
let checkForDangerousPath dangerousPaths =
  if Prefs.read Globals.confirmBigDeletes then begin
    if dangerousPaths <> [] then begin
      ...
      if Prefs.read Globals.batch then begin
          alwaysDisplay (f_ "Aborting...\n"); restoreTerminal ();
          exit Uicommon.fatalExit
      end else begin
        displayWhenInteractive (f_ "Do you really want to proceed? ");
        selectAction
          None
          [(["y"],
            s_ "Continue",
           ...
           (["n";"q";"x";""],
            ...
            (fun () -> alwaysDisplay "\n";
               restoreTerminal ();
               exit Uicommon.fatalExit))]
          (fun () -> display (f_ "Do you really want to proceed? "))
      end
    end
  end
...
let synchronizeOnce ?wantWatcher pathsOpt =
  ...
  if reconItemList = [] then begin
    ...
    (if anyEqualUpdates then
      Trace.status (s_ "Nothing to do: replicas have been changed only \
                    in identical ways since last sync.")
     else
       Trace.status (s_ "Nothing to do: replicas have not changed since last sync."));
...
let profmgrPref =
  Prefs.createBool profmgrPrefName false
    ~category:(`Basic `CLI)
    ~cli_only:true
    (s_ "interactive profile mode (text UI); command-line only")
    (s_ "Provide this preference in the command line arguments to enable \
...
let profmgrUsageMsg =
  Printf.sprintf
    (f_ "To start interactive profile selection, type \"%s -%s\".")
...
let scanProfiles () =
  ...
  Util.warnPrinter := Some (fun s -> alwaysDisplay (f_ "Warning: "); alwaysDisplay "%s\n\n" s);
...
let getProfile default =
  ...
  if (List.length !Uicommon.profilesAndRoots) > 10 then begin
    Trace.log (Format.sprintf (f_ "You have too many profiles in %s \
                for interactive selection. Please specify profile \
                or roots on command line.\n")
                Util.unisonDir);
    Trace.log (s_ "The profile names are:\n");
  ...
  let actions = Safelist.append
    ...
    (Safelist.map (fun (key, (profile, info)) ->
        ([key],
        Printf.sprintf (f_ "Profile: %s") profile,
  ...
  in
  let rec askProfile () =
    display (f_ "Select a profile ");
    selectAction None actions (fun () -> display (f_ "Select a profile "))
  in
  askProfile ();
  !selection
...
let rec start interface =
  if interface <> Uicommon.Text then
    Util.msg (f_ "This Unison binary only provides the text GUI...\n");
  begin try
