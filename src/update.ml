...
(* IMPORTANT!
   ...
   exactly the same as the type [archive] in version 2.51.4. This means that if
...
(* IMPORTANT!
   ...
   exactly the same result as [archiveName] would in version 2.51.4.
   ...
   This code is here only to support a smooth upgrade from versions <= 2.51.4
...
(* IMPORTANT!
   ...
   exactly the same result as [checkArchive] in version 2.51.4.
   If code changes elsewhere make this function produce a different result then
   it must be updated accordingly to again return the 2.51-compatible result. *)
...
(* IMPORTANT!
   ...
   capable of reading archives written by version 2.51.4. Be careful, as code
...
(* IMPORTANT! This val is here for smoother upgrades from versions <= 2.51.4
...
let removeArchiveLocal ((fspath: Fspath.t), (v: archiveVersion)): unit Lwt.t =
  ...
  (* IMPORTANT! This code is for smoother upgrades from versions <= 2.51.4
     It can be removed when this compatibility is no longer required. *)
