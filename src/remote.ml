...
let buildShellConnection shell host userOpt portOpt rootName termInteract =
  ...
  let (term, termPid) =
    ...
    | None ->
        (* Duplicate stderr to work around potential problems in case stderr
           could block (it's a socket, for example) and [shellCmd] sets it to
           non-blocking. This is known to happen with certain versions of
           OpenSSH. Which OpenSSH versions do this is not exactly clear but
           issues have been reported with at least versions < 8.7 and also 9.3.

           Issues arise specifically when systemd is used to launch Unison.
           systemd redirects Unison's stdout and stderr both to the same
           socket, which is capable of blocking. OpenSSH (certain versions)
           sets its stderr to non-blocking mode. Thus, if stderr would not
           have been duplicated then it and stdout (the same fd) would become
           non-blocking also for Unison, which in turn can cause the text UI
           to fail with an unhandled exception Sys_blocked_io (proxy for EAGAIN
           and EWOULDBLOCK). *)
        let err = Unix.dup ~cloexec:true Unix.stderr in
        ...
        let pid = Util.blockSignals [Sys.sigint] (fun () ->
          System.create_process shellCmd argsarray i1 o2 err) in
        let () = try Unix.close err with Unix.Unix_error _ -> () in
        let end_ssh () =
