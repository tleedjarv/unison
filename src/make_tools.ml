...
let () =
  if link_gettext = `OCaml || link_gettext = `Sys then begin
    ...
    | None when opam_var "gettext:installed" = "true" ->
        if link_gettext = `OCaml then begin
          "CAMLINCLS_GETTEXT" <-+= incl_opam_pkg_libdir "fileutils";
          "OCAMLLIBS_GETTEXT" <-+= "fileutils.cma"
        end;
        let libdir = opam_pkg_libdir "gettext" in
        List.iter (fun s -> "CAMLINCLS_GETTEXT" <-+= incl_quoted_dir s) [
            libdir;
            Filename.concat libdir "base";
            Filename.concat libdir "extension";
          ];
        "OCAMLLIBS_GETTEXT" <-+= "gettextBase.cma gettextExtension.cma";

        if link_gettext = `Sys then begin
          let libdir = opam_pkg_libdir "gettext-stub" in
          "CAMLINCLS_GETTEXT" <-+= incl_quoted_dir libdir;
          "OCAMLLIBS_GETTEXT" <-+= "gettextStub.cma";

          if ocaml_conf_var "ccomp_type" = "msvc" then begin
            (* This is needed until upstream ocaml-gettext is patched *)
            "CLIBS" <-+= "-cclib intl.lib";
          end;
        ...
        if link_gettext = `Sys then begin
          ...
          if build_macGUI then
            let ldflags_gettext =
              String.concat " " [
                inputs.$("LDFLAGS_GETTEXT");
                "-L" ^ (Filename.quote (Filename.concat ocaml_libdir "gettext-stub"));
                "-lgettextStub_stubs";
              ]
              |> double_escape_dquotes
            in
