      ...
      begin
        ...
        if shell ("{ printf '' > inotifytest__.ml ;" ^ ocamlc ^ " " ^
          ...
          if not_empty libinotify_inc then begin
            ...
            includes := (target ^ "CAMLCFLAGS_FSM_X = " ^ libinotify_inc) :: !includes
          end;
          if not_empty libinotify_lib then
            includes := ("CLIBS_FSM = " ^ libinotify_lib) :: !includes;
...
let run () =
  if Array.length Sys.argv > 2 then begin
    let cmd_and_args = Array.sub Sys.argv 2 (Array.length Sys.argv - 2) in
    let map_sep =
      if Sys.win32 then function '/' -> '\\' | c -> c else fun c -> c
    in
    cmd_and_args.(0) <- String.map map_sep cmd_and_args.(0);
    Tools.exec (Array.to_list cmd_and_args)
  end
