...
ACE inheritance may in certain scenarios cause synchronization inconsistencies.
In Windows, only explicit ACEs are synchronized; inherited ACEs are not
actively synchronized, but Windows will propagate ACEs from parent directories
(unless inheritance is explicitly prevented on a file or a directory---this
prevention is also synchronized). Due to inheritance, the ultimately effective
ACL may be different, or provide different access, even after synchronization.
...
values. Unison synchronizes only up to 4KB of each attribute value.
\item Attributes are synchronized as name-value pairs. On Linux, attribute
names always have a fully qualified form (\texttt{namespace.attribute}).
Other platforms do not have the same constraint.
The consequence of this is that while attribute names from Linux can be
propagated to other platforms and back to Linux (e.g. in ``star topology''),
native attribute names from other platforms will be rejected on Linux.
\end{itemize}

\verb|xattrignore| preference can be used to filter the names of extended
attributes that will be synchronized.
